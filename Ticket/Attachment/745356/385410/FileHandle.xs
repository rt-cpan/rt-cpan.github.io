...
#endif
    return (long)PerlIO_tell(file->file);
}
...
NYTP_open(const char *name, const char *mode) {
    PerlIO *raw_file = PerlIO_open(name, mode);
    NYTP_file file;

    if (!PerlIOValid(raw_file))
        return NULL;
...
grab_input(NYTP_file ifile) {
    ...
    while (1) {
        ...
        if (ifile->zs.avail_in == 0 && !ifile->stdio_at_eof) {
            size_t got = PerlIO_read(ifile->file, ifile->small_buffer, 
                               NYTP_FILE_SMALL_BUFFER_SIZE );
...
NYTP_read_unchecked(NYTP_file ifile, void *buffer, size_t len) {
    ...
    if (FILE_STATE(ifile) == NYTP_FILE_STDIO) {
        return PerlIO_read(ifile->file, buffer, len );
    }
#ifdef HAS_ZLIB
...
NYTP_gets(NYTP_file ifile, char **buffer_p, size_t *len_p) {
    ...
    {
	     SV * line = newSVpvn(buffer, prev_len);
	     char * strptr = sv_gets(line, ifile->file, 1);
	     if (strptr != NULL) {
	     	  STRLEN actlen;

	    	  sv_catpvn(line, "", 1); /* add '\0' terminator */
	     	  actlen = SvCUR(line);
	
	     	  if (len < actlen) {
	     	  	buffer = (char *)saferealloc(buffer, actlen);
	     	  }
	        Copy(SvPVX(line), buffer, actlen, char);
					--actlen;
...
sync_avail_out_to_ftell(NYTP_file ofile) {
    const long result = PerlIO_tell(ofile->file);
    ...
    while (1) {
        ...
        if (status == Z_OK || status == Z_STREAM_END) {
            if (ofile->zs.avail_out == 0 || flush != Z_NO_FLUSH) {
                ...
                while (avail > 0) {
                    size_t count = PerlIO_write(ofile->file, where, avail );
...
NYTP_write(NYTP_file ofile, const void *buffer, size_t len) {
    ...
    if (FILE_STATE(ofile) == NYTP_FILE_STDIO) {
        ...
        if (len == 0)
            return len;
        if (PerlIO_write(ofile->file, buffer, len ) < 1) {
            dTHX;
            croak("fwrite error %d writing %ld bytes to fd%d: %s",
                errno, (long)len, PerlIO_fileno(ofile->file), strerror(errno));
        }
        return len;
    }
#ifdef HAS_ZLIB
...
NYTP_printf(NYTP_file ofile, const char *format, ...) {
    ...
    retval = PerlIO_vprintf(ofile->file, format, args);
    va_end(args);
