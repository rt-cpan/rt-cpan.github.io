...
sub new {
  ...
  # It may overide normal options if tags conflict!
  my %ssl_args;
  %ssl_args = %{$arg->{SSL_Advanced}} if (ref ($arg->{SSL_Advanced}) eq "HASH");
...
#    $FTPS_ERROR->autoflush (1);
  ...
  if ( $encrypt_mode eq CLR_CRYPT ) {
     ...
     # by the SSL_Advanced functionality.
     # ------------------------------------------------------------------------
  ...
  ${*$obj}{myAdvanced}   =  \%ssl_args  if (ref ($arg->{SSL_Advanced}) eq "HASH");
...
sub _open_data_channel {
  ...
  # We don't care about any advanced features here, only in _get_data_channel().
...
sub _get_data_channel {
   ...
   my $io;
   if ( ${*$self}{data_prot} eq DATA_PROT_PRIVATE && exists (${*$self}{myAdvanced}) ) {
      my %advanced = %{${*$self}{myAdvanced}};
      my $mode = $advanced{SSL_version};

      $io = IO::Socket::SSL->start_SSL ( ${*$self}{data_ch}, \%advanced )
