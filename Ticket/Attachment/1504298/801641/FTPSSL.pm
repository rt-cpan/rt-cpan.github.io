...
sub new {
  ...
  if (ref ($arg->{SSL_Client_Certificate}) eq "HASH") {
     ...
     %ssl_args = %{$arg->{SSL_Advanced}};       # Depreciated in v0.18 (in 2011) and fatal in v0.26 (2015)
     $ERRSTR = "SSL_Advanced is no longer supported!  Please use option SSL_Client_Certificate instead!";
     croak ("\n" . $ERRSTR . "\n");
...
#    $FTPS_ERROR->autoflush (1);
  ...
  if ( $encrypt_mode eq CLR_CRYPT ) {
     ...
     $ssl_args{SSL_session_cache} = IO::Socket::SSL::Session_Cache->new (4);
     # $ssl_args{SSL_session_key} = "abcd1234";

{ my $x = $socket;  bless ( $x, $type ); _debug_print_hash ( $x, "Before start_SSL() call", "initialization", $encrypt_mode, \%ssl_args ); }
     ...
     # Set the session key from the info in the session cache ...
     if ( exists ${*$obj}{_SSL_arguments}->{SSL_session_cache} ) {
        ${*$obj}{_SSL_arguments}->{SSL_session_key} = 
                  ${*$obj}{_SSL_arguments}->{SSL_session_cache}->{_head}->{key};
     }
  }
  ...
  if ( defined $fixHelp ) {
     ...
     ${*$obj}{OverrideHELP} = 0;     # So we know OverrideHELP was used ...
  ...
  ${*$obj}{OverridePASV} = $pasvHost  if (defined $pasvHost);
...
sub quot {
   ...
   } elsif ( $cmd2 eq "HELP" && exists ${*$self}{OverrideHELP} ) {
...
sub _debug_print_hash
{
   ...
   foreach (@lst) {
      ...
      $self->_print_hash_tree ( "  ", 0, $_, $val, \@seps, \%loop );
   }
...
sub _print_hash_tree
{
   ...
   if ( $val =~ m/ARRAY\(0/ ) {
      ...
      if ( exists $loop_ref->{$key} ) {
         $self->_print_LOG (" ... Infinite Hash Loop Detected!\n");
      } else {
         $start{$key} = $loop_ref->{$key} = $val;
         $self->_print_LOG ( "\n" );
         foreach (sort keys %{$val}) {
            %{$loop_ref} = %start;
            $self->_print_hash_tree ( $indent, $lvl + 1, $_, $val->{$_},
                                      $sep_ref, $loop_ref );
         }
      }
...
if the following 5 I<FTP> commands are allowed: B<ALLO>, B<NOOP>, B<HELP>,
B<MFMT> and B<MDTM>.  The B<ALLO> command is checked during all I<put>/I<get>
command sequences.  The other four commands are not checked as often.  The
functions I<xput> and I<xtransfer> also tests for four more commands: B<STOR>,
B<DELE>, B<RNFR> and B<RNTO>.
...
L<http://search.cpan.org/~cleach/Net-FTPSSL-0.28/FTPSSL.pm> or
...
Copyright (c) 2009 - 2015 Curtis Leach. All rights reserved.
