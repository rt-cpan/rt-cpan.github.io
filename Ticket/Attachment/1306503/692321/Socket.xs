...
inet_ntop(af, ip_address_sv)
...
#ifdef WIN32
	/*
	 * The MSWin inet_XtoY() implementation first became available under Vista, and additionally
	 * the declarations for inet_XtoY() are missing in the MINGW and CYGWIN header <ws2tcpip.h>.
	 * The WSA stuff WSAAddressToStringA() and WSAStringToAddressA() already exists
	 * since Windows 2000, although Windows 2000 has no IPv6 support.
	 */
	if (af == AF_INET) {
	  DWORD slen = sizeof(str);
	  struct sockaddr_in sockaddr;

	  Zero(&sockaddr, sizeof(sockaddr), char);
	  sockaddr.sin_family = AF_INET;
	  Copy(ip_address, &sockaddr.sin_addr, addrlen, char);
	  if (WSAAddressToStringA((struct sockaddr *)(&sockaddr), sizeof(sockaddr), NULL, str, &slen))
	     *str = '\0';
	}
#ifdef AF_INET6
	else { /* Must be AF_INET6 ... checked above */
	  DWORD slen = sizeof(str);
	...
	if (!inet_ntop(af, &addr, str, sizeof str))
	    *str = '\0';
#endif
...
inet_pton(af, host)
	...
	if (ok)
	{
	  ...
	  if (af == AF_INET) {
	    ...
	    ok = (!WSAStringToAddressA((char *)host, af, NULL, (struct sockaddr *)(&sockaddr), &alen) && alen>0);
	    if (ok)
	      Copy(&sockaddr.sin_addr, &ip_address, alen, char);
	    else
	      croak("inet_ntop/WSAStringToAddress(%s) failed: E-%d",host,GetLastError());
	  }
#ifdef AF_INET6
	  else { /* Must be AF_INET6 ... checked above */
	    struct sockaddr_in6 sockaddr;
