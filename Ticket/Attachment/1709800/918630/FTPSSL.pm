...
BEGIN {
    $VERSION = "0.36";              # The version of this module!
    ...
    $debug_log_msg = "\n"
                   ...
                   . "IO-Socket-INET Version: $IO::Socket::INET::VERSION\n"
                   . "IO-Socket-SSL Version: $IO::Socket::SSL::VERSION\n\n"
                   . "${msg}\n\n"
                   . "Perl: $pv,  OS: $^O\n\n";
...
sub new {
  ...
  if ( $debug && defined $arg->{DebugLogFile} ) {
     $use_logfile  = (ref ($arg->{DebugLogFile}) eq "" &&
                      $arg->{DebugLogFile} ne "");
     $use_glob     = (ref ($arg->{DebugLogFile}) eq "GLOB");
  }
...
if ( $debug ) { $IO::Socket::SSL::DEBUG = 3; $Net::SSLeay::trace = 4; } ### BETA
...
_print_LOG (undef, "Calling start_ssl()\n")  if ( $debug );  ### BETA
...
_print_LOG (undef, "After start_ssl()\n")  if ( $debug );  ### BETA
  ...
  if ( defined $fixHelp ) {
     my %helpHash;
_print_LOG (undef, "Overriding Help ()\n")  if ( $debug );  ### BETA
  ...
  # --------------------------------------
$obj->_print_LOG ("Setting control vars in hash!\n")  if ( $debug );  ### BETA
...
$obj->_print_LOG ("Setting Context/Session info!\n")  if ( $debug );  ### BETA
  ...
  # -------------------------------------------------------------------------
$obj->_print_LOG ("Printing the object details!\n")  if ( $debug );  ### BETA
  if ( $debug ) {
     if ( ref ($arg->{SSL_Client_Certificate}) eq "HASH" ) {
...
print STDERR "In prep_data_channel ([", join ("],[", @_), "]) --Crypt = [${*$self}{_FTPSSL_arguments}->{Crypt}]\n"; ### BETA
...
print STDERR "In Common_Put([$self], [$file_loc], [$file_rem], [$offset])\n  Info: ${*$self}{_FTPSSL_arguments}\n";  ### BETA
...
sub _mfmt {
  ...
  my $GMT_flag = ( ${*$self}{_FTPSSL_arguments}->{FixPutTs} >= 0 );
  if ( defined $local ) {
...
sub _mdtm {
  ...
  my $timestamp;
  my $GMT_flag = ( ${*$self}{_FTPSSL_arguments}->{FixGetTs} >= 0 );
  if ( defined $local ) {
     $GMT_flag = $local ? 0 : 1;    # Override PreserveTimestamp option ...
  }
...
sub _help {
   ...
   # For the other supported variants to OverrideHELP!
      ...
      $arg->{help_cmds_no_syntax_available} = 1;
      my $ft = $self->_feat();   # Just makes sure it's cached!
      unless ( exists $arg->{help_SITE_msg} ) {
         $self->_site_help ( $arg->{help_cmds_found} );
      }
   }
...
sub _feat {
   ...
   if (exists $arg->{help_FEAT_msg2}) {
      $arg->{last_ftp_msg} = ${*$self}{_FTPSSL_arguments}->{help_FEAT_msg2};
...
L<http://search.cpan.org/~cleach/Net-FTPSSL-0.36/FTPSSL.pm> or
