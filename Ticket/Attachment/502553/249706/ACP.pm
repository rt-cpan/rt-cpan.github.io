...
__PACKAGE__->mk_accessors(qw( owner_id owner_displayname acl account ));

=head1 NAME

Net::Amazon::S3::ACP - convenience object for working with Amazon S3 Access Control Policies/Lists
...
=head1 SYNOPSIS
  ...
  my $acl = $bucket->get_acl(); # gets ACP/ACL in XML format
  my $acp = $bucket->get_acp(); # gets ACP/ACL as an object

  # Owner info
  my $owner_id = $acp->owner_id();
  my $owner_displayname = $acp->owner_displayname();

  while (my ($name, $grant) = each %{$acp->acl()}) {
     print "Policy for '$name':\n";
     print "   Type: $acp->{type}\n";
     if ($acp->{type} eq 'ID') {
        print "   AWS ID: $acp->{id}\n";
        print "   AWS Display Name: $acp->{displayname}\n";
     }
     elsif ($acp->{type} eq 'email') {
        print "   email address: $acp->{email}\n";
     }
     elsif ($acp->{type} eq 'URI') {
        print "   group definition URI: $acp->{URI}\n";
     }
     print '   Permissions: ', join(', ', @{$grant->{permissions}}), "\n";
  }

  $acp->clear(); # wipe all grants in ACP object
  ...
  $acp->delete($ID); # remove whole ACL for given ID
  $acp->delete($ID => 'READ'); # remove this permission only
  $acp->delete($ID => [qw( READ WRITE )]); # remove these permissions only

  $bucket->set_acp({acp => $acp});
  $bucket->set_acp({acp => $acp, key => 'whatever'});

  # If you need to play with this directly
  my $acp = Net::Amazon::S3::ACP->new();
  $acp->parse($xml);
  print $acp->stringify(); # stringifies to XML

  # If you have a Net::Amazon::S3 account object, pass it in!
  my $s3 = Net::Amazon::S3->new({...});
  my $acp = Net::Amazon::S3::ACP->new({account => $s3});

=head1 DESCRIPTION

This module represents an S3 Access Control Policy. There is practically
no difference between an Access Control Policy (ACP) and an Access
Control List (ACL), at least in Amazon's documentation, except that what
they call ACL is actually an ACP.

In the context of Net::Amazon::S3, we'll refer to the ACL when we speak
about the XML form of the ACP, and to the ACP when we deal with
Net::Amazon::S3::ACP objects. In any case, a N::A::S3::ACP is nothing
more than a representation of the XML ACL that is easier to handle
in order to modify/set the Access Control Policy of a given resource. As
such, there are methods that ease passing from one representation to the
other, namely L</parse> (to parse an XML document into an object) and
L</stringify> (to get the XML representation of the ACP).

An ACP is supposed to be compound of two main parts:
...
the list of grants that are associated to the resource. The acl is
actually a hash reference in which the keys identify the grantee, and
the values are hash references with the details.
...
As a general note, you don't need to bother with acl internals if you
want to set it: just use the L</add>, L</delete> and L</clear>
convienience methods, that try to DWIM.

=back

Both parts can be taken with the correspondent accessors.

=head1 METHODS

=head2 new

Create a new ACP object. Expects a hash containing these two arguments:

=over

=item account (optional)

a Net::Amazon::S3 object. Net::Amazon::S3::ACP never accesses the network
...
the ACL, i.e. the list of grants for this ACP. You shouldn't normally
...
the acl coming from another N::A::S3::ACP object).
...
Parse and XML document and fills the ACP. The previous contents
of the ACP are wiped, if any.

Expects a single string with the XML document to parse.

Returns a referenct to the ACP object.
...
sub _parse_acl {
   ...
   return {map { $self->_parse_acl_grant($xpc, $_); }
        $xpc->findnodes('.//s3:AccessControlList/s3:Grant')};
} ## end sub _parse_acl

sub _parse_acl_grant {
   my ($self, $xpc, $node) = @_;
...
Renders the ACP object as an XML document that can be sent to S3.
...
sub stringify {
   ...
   my $acl_chunk   = $self->_stringify_acl();

   # Indent for pretty printing
   s/^/   /mxsg for $owner_chunk, $acl_chunk;

   return <<"END_OF_ACP";
<?xml version="1.0" encoding="UTF-8"?>
<AccessControlPolicy>
$owner_chunk$acl_chunk
</AccessControlPolicy>
END_OF_ACP
} ## end sub stringify
...
sub _stringify_acl {
   ...
   my $list = join "\n",
     map { $self->_stringify_grant($_); } values %{$self->acl()};
...
Gets the canonical representation for an item in the ACL.
...
Adds permissions for a given series of grantee.
...
sub add {
   my $self = shift;
   my @acls = ref($_[0]) ? %{$_[0]} : @_;
   while (my ($target, $item) = splice @acls, 0, 2) {
...
sub _add {
   ...
   my $acl = $self->acl();
   $self->acl($acl = {}) unless $acl;

   my $previous = $acl->{$target} ||= $item;
   ...
   delete $acl->{$target} unless @{$previous->{permissions}};
...
Removes permissions for a given series of grantee.
...
sub _delete {
   ...
   my $acl = $self->acl() or return;
   my $perms = $item->{permissions} || [];
   if (scalar(@$perms)) {
      my $previous = $acl->{$target} or return;
...
Wipes away the ACL part of this ACP, i.e. removes all grants.
