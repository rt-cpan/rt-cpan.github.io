...

{
   ...
   is($acp->owner_id(),          'yadda',    'owner_id');
   is($acp->owner_displayname(), 'whatever', 'owner_displayname');

   ok(my $acl = $acp->acl(), 'acl exists');

   is(scalar(keys %$acl), 3, 'acl has right number of elements');

   {
      ok(exists($acl->{yadda}), 'ID yadda is in acl');
      ok(my $by_id = $acl->{yadda}, 'yadda points to something');
   ...
   {
      ok(exists($acl->{'foo@example.com'}), 'email is in acl');
      ok(my $by_email = $acl->{'foo@example.com'},
   ...
   {
      my ($anonkey) = grep { /AllUsers/ } keys %$acl;
      ok($anonkey, 'anonymous is in acl');
      like($anonkey, qr{\A http://}mxs, 'anonymous is specified by URI');
      ok(my $by_uri = $acl->{$anonkey}, 'anonymous points to something');
   ...
   # Delete anonymous
   $acp->delete('*');
   is(keys(%$acl), 2, 'one less key after deletion');
   ok(! scalar(grep { /AllUsers/ } keys %$acl), 'anonymous was deleted');

   $acp->delete('yadda');
   $acp->delete('foo@example.com');
   is(keys(%$acl), 0, 'no more grants in acl');
   ...
   {
      ok(exists($acl->{foo}), 'ID foo created in acl');
      ok(my $by_id = $acl->{foo}, 'foo points to something');
   ...
   {
      ok(exists($acl->{'bar@example.com'}), 'email created in acl');
      ok(my $by_email = $acl->{'bar@example.com'},
         'email points to something');
      is($by_email->{type}, 'email', 'email is of the right type');
   ...
   {
      my ($anonkey) = grep { /AuthenticatedUsers/ } keys %$acl;
      ok($anonkey, 'anonymous is in acl');
      like($anonkey, qr{\A http://}mxs, 'anonymous is specified by URI');
      ok(my $by_uri = $acl->{$anonkey}, 'anonymous points to something');
      is($by_uri->{type}, 'URI',    'anonymous is of the right type');
   ...
   while (my ($main, $variants) = each %variants) {
      for my $variant (@$variants) {
         $acp->delete('foo');
         ok(! $acl->{foo}, 'foo deleted');
         $acp->add(foo => $variant);
         ok($acl->{foo}, 'foo re-added');
         is($acl->{foo}{permissions}[0], $main, "$main permission");
      }
   }

   my $xml_out = $acp->stringify();
   for my $regex (
      qr/AllUsers/,
