...
BEGIN {
  ...
  if ( $nbr > 0 ) {
    ...
    foreach (@modules_to_install) { print "\t- $_\n"; }
    print "\n Try : "
      . 'perl -MCPAN -e "install '
      . join( $SPACE, @modules_to_install )
      . $DOUBLE_QUOTE . "\n";
    exit;
  }
}
...
sub display_size_notebook {
  ...
  my $button_add_files = $labframe_filereplib->ColoredButton(
    ...
    -command => sub {
      foreach my $file ( get_file( $labframe_filereplib, undef, 1 ) ) {
  ...
  my $button_add_directories = $labframe_filereplib->ColoredButton(
    ...
    -command => sub {
      if ( my $dir = get_dir($labframe_filereplib) ) { $add_files_rep_listbox->insert( 'end', "d: $dir\n" ); }
    },
  );
...
sub display_general_notebook {
  ...
  my $entry_iconfile
    = get_entry_widget( $labframe_general, { textvariable => \$options_pp{'--icon'}{-value} } );
...
Copyright 2003, 2004, 2005, 2006, 2011 by Doug Gruber E<lt>doug(a)dougthug.comE<gt>,
