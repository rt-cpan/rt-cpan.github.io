...
sub do_http_request {
    ...
    my ( $method, $url, $newheaders, $content, $save_to, $callback_func,
        $chunk ) = HTTP::DAV::Utils::rearrange(
        [
    ...
    ####
    # Do the HTTP call
    my $req = HTTP::Request->new( $method, $url_obj, $headers->to_http_headers,
        $content );
...
sub credentials {
    ...
    my ( $user, $pass, $url, $realm ) =
      HTTP::DAV::Utils::rearrange( [ 'USER', 'PASS', 'URL', 'REALM' ], @p );
...
# The standard UserAgent doesn't.
{
    ...
    sub credentials {
		...
		# No previous credentials and no new user/pass: return undef
		if (! $cred) {
			return;
		}
	...
	# Override and disallow redirects. See RT #19616
	sub redirect_ok {
        return 0;
	}
