...
my $r_cannotfindfile=qr/not find the file?|Не удается найти указанный файл/i;
my $r_accessdenied=qr/access is denied?|Отказано в доступе/i;
my $r_fileexists=qr/file exists?|Файл существует/i;
my $r_invalidhandle=qr/handle is invalid?|Неверный дескриптор/i;
my $r_permdenied=qr/permission denied/i;
my $r_alreadyexists=qr/file already exists?|Невозможно создать файл, так как он уже существует/i;
my $r_cannotcreate=qr/cannot create/i;
my $r_nosuchfile=qr/no such file|Не удается найти указанный файл/i;
