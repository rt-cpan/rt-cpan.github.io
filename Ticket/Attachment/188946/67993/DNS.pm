...
sub wire2presentation {
    ...
    #MKL, 2006_04_21
    #added basic CCSID conversion table just to translate numbers,"-"sign and non-capital letters
    #This CCSID conversion table translates from ISO 8859-1 to EBCDIC IBM-1047
    my %iconv = (
                  45 => 96,
                  46 => 75,
                  48 => 240,
	...
	my $char=unpack("x".$i."C1",$wire);
        ...
        #MKL: Changed 2006_04_21
        #MKL: Test about EBCDIC issue. --- I am not sure if it is correct.
        #MKL:                          --- This is just a first guess!!!
        #MKL: This modification works for translating found MX records.
        #MKL: I think this will not work for other DNS answers because I deleted 
        #MKL: handling of escaped characters.
        if ( $char==ord (".") ){
          $presentation.= "." ;
        } else {
          #MKL translate character
          my $ichar=$iconv{$char};
          $presentation.=chr($ichar);
        }
	#MKL if ( $char < 33 || $char > 126 ){          
        #MKL  !! checking CCS entries to test for certain characters is never a good choice !!
	#MKL     $presentation.= sprintf ("\\%03u" ,$char);
        #MKL The following looks much better because the ord() method shows the correct CCS number
	#MKL }elsif ( $char == ord( "\"" )) {   
	#MKL     $presentation.= "\\\"";    
	#MKL }elsif ( $char == ord( "\$" )) {   
	#MKL     $presentation.= "\\\$";    
...
sub presentation2wire {
    ...
    #MKL, 2006_04_21
    #MKL added basic CCSID conversion table just to translate numbers,"-"sign and non-capital letters
    #MKL This CCSID conversion table translates from EBCDIC IBM-1047 to ISO 8859-1
    my %iconv = (
                  75 => 46,
                  96 => 45,
                  129 => 97,
	...
	if (  $char == ord ('\\')){
		...
		$i+=1;
            ...
            #MKL: Changed 2006_04_21
            #MKL: Test about EBCDIC issue. --- like above I am not sure if it is correct.
            #MKL:                          --- This is just a first guess!!!
            my $ichar=$iconv{$char};
	    $wire .=  pack("C",$ichar);  
	    #MKL: old one: $wire .=  pack("C",$char);  
        }
	$i++;
    }
    return $wire;
