...
sub new {
  ...
  # Used to emulate bug # 73115 (Boolean flag)
  my $emulate_bug  = $arg->{EmulateBug} || 0;
...
$self->_print_LOG ("====== In prep_data_channel past PBSZ & PROT calls! =======\n");
...
$self->_print_LOG ("====== In put ($file_loc, $file_rem) --- Calling prep_data_channel () ======\n");
  unless ( $self->prep_data_channel() ) {
    close ($localfd)  if ($close_file);
$self->_print_LOG ("====== Call to prep_data_channel failed! =======\n");
    return undef;    # Already decided not to call croak if you get here!
  }
$self->_print_LOG ("====== Call to prep_data_channel worked! =======\n");
...
sub response {
  ...
  # So I can test bug fix (bug # 73115) without having access to a server
  # that behaves that way!
  if (${*$self}{EmulateBug}) {
     ${*$self}{last_ftp_msg} =~ s/^(.)/$1\n/;
     if ( ${*$self}{debug} ) {
        _print_LOG ( $self, "(bug-fix-bug-fix-but-fix)\n" );
        _print_LOG ( $self, ${*$self}{last_ftp_msg} );
        _print_LOG ( $self, "\n(bug-fix-bug-fix-but-fix)\n" );
     }
  }

_print_LOG ($self, "====== (" . substr( ${*$self}{last_ftp_msg}, 0, 1 ) . ") =======\n");
...
Copyright (c) 2009 - 2011 Curtis Leach. All rights reserved.
