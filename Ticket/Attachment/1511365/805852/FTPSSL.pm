...
sub _print_LOG
{
   ...
   if ( defined $self && exists ${*$self}{ftpssl_filehandle} ) {
      $FILE = ${*$self}{ftpssl_filehandle};     # A custom log file ...
...
channel> session for all I<data channel> connections.  (0/1)
...
this server also does load balancing, you are out of luck.  This option can't
help you if multiple IP Addresses are returned.
